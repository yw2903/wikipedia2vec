...
cdef class LinkGraph:
    ...
    @staticmethod
    def build(dump_db, myylmdb, dictionary, pool_size, chunk_size, progressbar=True):
...
def _process_page_2(unicode title, int32_t offset):
    cdef int32_t page_index, dest_index
    cdef list dest_indices
    #cdef Paragraph paragraph
    #cdef WikiLink link
    ...
    dest_indices = []
    env = lmdb.open('mylmdb', map_size = 1e9, max_dbs = 1000)
    db = env.open_db(key = "mylmdb".encode(), dupsort = True)
    with env.begin(db) as txn:
        for key in db.get_keys():
            ...
            for (key, value) in cursor.iternext_dup(keys=True, values=True):
                links.append(pickle.loads(value)[0])

                for link in links:
                    dest_index = _dictionary.get_entity_index(link)
                    if dest_index != -1:
                        dest_indices.append(dest_index - offset)
